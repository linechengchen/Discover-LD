 ...
 */
class SuperMenu extends Model implements Sortable
{
    ...
    {
        $connection = config('super-admin.database.connection') ?: config('database.default');

        $this->setConnection($connection);

        $this->setTable(config('super-admin.database.menu_table'));
    }
    ...
    {
        $pivotTable = config('super-admin.database.role_menu_table');

        $relatedModel = config('super-admin.database.roles_model');
    ...
    {
        $pivotTable = config('super-admin.database.permission_menu_table');

        $relatedModel = config('super-admin.database.permissions_model');
    ...
    {
        return config('super-admin.menu.bind_permission') && config('super-admin.permission.enable');
    }
    ...
    {
        return (bool) config('super-admin.permission.enable');
    }
